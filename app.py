...
"""
    ...
    if resp.status_code != 200:
        # Surface the actual Google error detail (it won't include the API key) so we can debug 403 quickly.
        detail = resp.text[:1200]
        _log("D", "app.py:146", "Gemini non-200", {"status": resp.status_code, "detail": detail[:500]})
